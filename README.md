...
- 공기질 SSE 실시간 스트림
- 조도 센서 현재 상태 gRPC polling producer
- 조도 리포트 저장 queue/consumer
- 조도와 최근 공기질 기준 공기청정기 자동제어
- 사용자 overview shortcut 조회·수정·기본값 재설정 API
- 관리자용 default overview shortcut SSR 조회·저장 화면
- 관리자용 API 문서 공개/비공개 토글
- 관리자용 자동제어 정책 SSR 조회·수정·활성화 토글
...
- 임베디드 gRPC 서버가 떠 있지 않으면 공기질/조도 producer가 연결 실패 로그를 남기고 백오프 delay를 적용합니다.
...
├─ V8__create_control_action_log.sql
├─ V9__create_control_setting.sql
├─ V10__rename_bad_air_quality_count_to_ratio.sql
└─ V11__add_clean_air_quality_ratio_threshold.sql
...
├─ lightsensor/v1/lightsensor.proto
...
- PostgreSQL
- Redis
- `sensor.v2.SensorService`, `lightsensor.v1.LightSensorService`, `airpurifier.v1.AirPurifierService`를 제공하는 gRPC 서버
...
- `EMBEDDED_SERVER_HOST`
  - 임베디드 gRPC 서버 host입니다.
- `EMBEDDED_SERVER_GRPC_PORT`
...
- `DEV_GRAFANA_URL` 기본값 `http://localhost:${DEV_GRAFANA_PORT}`
...
PROD_GRAFANA_URL=https://monitoring.replace-with-server.example.com
```
...
- `V8`: `light_report.measured_at` 조회 인덱스 및 `control_action_log` 테이블 생성
- `V9`: 공기청정기 자동제어 정책 저장용 `control_setting` 테이블 생성
- `V10`: 자동제어 정책 컬럼명을 detection threshold/ratio 의미로 변경
- `V11`: 깨끗한 공기질 비율 기준 `clean_air_quality_ratio_threshold` 추가
...
gRPC 조회 실패 시 공기질 producer는 백오프 delay를 적용합니다.

## 조도 수집 및 자동제어 흐름

1. `LightProducer`가 앱 시작 후 조도 센서 gRPC 서버를 polling합니다.
2. 조회한 원시 조도 데이터를 `SaveLightReportQueue`에 저장 명령으로 넣습니다.
3. `LightConsumer`가 명령을 소비해 `light_report` 테이블에 저장합니다.
4. `AirQualityProducer`가 공기청정기 현재 상태를 조회한 뒤 자동제어 명령을 queue에 넣습니다.
5. `AutoControlAirPurifierConsumer`가 조도 리포트와 최근 공기질 timeseries를 기준으로 공기청정기 ON/OFF 여부를 판단합니다.
6. 제어가 실행되면 `airpurifier.v1.AirPurifierService`로 전원 제어 RPC를 호출하고 `control_action_log`에 결과 사유를 저장합니다.

조도 gRPC 조회 실패 시 조도 producer도 백오프 delay를 적용합니다.
...
- 조도 리포트는 자동제어 판단에 사용되며, 현재 별도 공개 조회 API는 없습니다.
...
- 공기청정기 자동제어는 관리자 화면에서 활성화 여부와 판단 기준을 관리합니다.
...
자동제어 정책은 어두움/밝음 lux 기준, 어두움/밝음 지속 시간, 공기질 확인 구간, 나쁜 공기질 비율, 깨끗한 공기질 비율, 자동제어 쿨타임을 관리합니다. 기본 row가 없으면 애플리케이션 시작 시 기본 정책을 생성합니다.
...
- prod: `PROD_GRAFANA_URL` (기존 TLS proxy가 HTTPS를 종료하고 Grafana 포트로 전달)
- 계정: `admin`
...
이 저장소는 중앙 백엔드의 공기질 수집·조회 흐름, 조도 리포트 저장, 공기청정기 자동제어, overview shortcut 기능을 구현 중입니다. gRPC polling, DB 저장, Redis refresh token 상태 저장, materialized view 기반 timeseries 조회, SSE 실시간 전송, 사용자 overview shortcut 조회·수정·재설정, 관리자 SSR 화면, 자동제어 정책 관리, SSO 로그인, JWT 쿠키 인증, Scalar/OpenAPI 문서와 Redis 기반 공개 토글, Whozin 공개 회원 조회 연동, Actuator 메트릭 노출은 구현되어 있습니다.
